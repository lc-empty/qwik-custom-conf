...
# Kitty conf --------------------------------
apply_terminal() {
	local KITTY_CONF="${PATH_TERM}/kitty.conf"

	sed -i "s/include themes\/catppuccin-[^m]*.conf/include themes\/catppuccin-${THEME}.conf/" "${KITTY_CONF}"
}
...
apply_dunst() {
	# modify dunst config
	sed -i ${PATH_BSPWM}/dunst/dunstrc \
